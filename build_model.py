...
def cnn():
    ...
    with tf.name_scope('data'):
        x = tf.placeholder(tf.float32 , shape = [None, image_dim * image_dim * channels], name = 'Input')
        y = tf.placeholder(tf.float32, shape = [None, num_labels],name = 'Output')
        x_image = tf.reshape(x,[-1 , image_dim, image_dim, channels],name = 'images')
